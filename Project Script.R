...
for(i in 1:k){
  ...
  RF.fit<-randomForest(as.factor(Survived)~Sex + Pclass + Age + SibSp + Parch + Fare + Embarked + Title + FamilySize + FamilyID2, data=train.set[-ind,], importance=TRUE, ntree=2000)
...
# Logistic Regression
LR.fit<-glm(Survived~Sex*Age + Sex*FamilySize + Sex*Embarked + Sex*Pclass + Sex*Fare + Sex*Title + Pclass*Fare + Pclass*Age + Pclass*FamilySize + Pclass*Embarked + Pclass*Title + Age*Fare + Age*Embarked + Age*Title + Age*FamilySize + Fare*Embarked + Fare*Title + Fare*FamilySize + Embarked*Title + Embarked*FamilySize + Title*FamilySize, data=train.set ,family=binomial)
...
# Cross-validations + SVM
model<-Survived~Sex + Pclass + Age + SibSp + Parch + Fare + Embarked + Title + FamilySize + FamilyID2
rows<-nrow(train.set)
# 10-fold
k<-10
score<-0
...
write.csv(RF.result, file="result.csv",row.names = F, quote = F)
